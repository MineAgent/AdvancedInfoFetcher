 ...
 * per server (this mod's, plus one for every other mod that uses it, mcctl for example). Any such
 * thread left behind makes {@code DestroyJavaVM} wait; 15 seconds later the watchdog writes a bogus
...
public final class ClientExitWatcher {
	...
	public static void onClientExit(Runnable onExit) {
		Thread watcher = new Thread(() -> {
			...
			LOG.info("client exited, stopping the advanced-info-fetch server");
			try {
				onExit.run();
			} catch (Throwable t) {
